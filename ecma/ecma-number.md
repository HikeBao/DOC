...
Q：为什么`0.1+0.2 !== 0.3`,有什么方法解决？

A：因为JS解析引擎对浮点数加减运算存在一定的误差，所以只要增加一个误差范围即可解决问题。`Number.EPSILON`就是`Number`一个最小误差值。如果` (0.3 - ( 0.1 - 0.2)) < Number.EPSILON`的话，我们就认为`0.3` 等于`(0.1 + 0.2)`

<br>

Q：现在`Number`和`window`对象都内置了相同的方法和属性(`parseInt、parseFloat`)，那么我们应该什么时候使用`window`的？什么时候使用`Number`的？

A：首先要清楚一个事实就是内置在`Number`对象上面的方法和属性都是在`ES6`后提出来的，所以是不兼容IE浏览器的，如果业务场景不需要考虑兼容`IE`的话可以直接使用`Number`上面的方法，否则就使用`window`上的内置方法
